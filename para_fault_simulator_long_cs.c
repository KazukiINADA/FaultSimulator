...
void logic_cal_bit(int (*signal_line)[6],long long int *signal_bit,int line_no,unsigned int *pointa_list);
...
int main (int argc, char *argv[]){
  ...
  long long int **test_ptn;
  unsigned int test_ptns;
  unsigned int ptn_i;
  unsigned int test_arrays;

  unsigned int (*fault_ptn)[2];
  unsigned int fault_ptns;
  unsigned int fault_i;

  long long int *signal_bit;

  long long int **true_out;
  ...
  strcpy(filename,"/mnt/tiger/home/test_db/comb/benchmark/iscas89_cs/");
  ...
  //テストパターン読み込み
  strcpy(filename,"/mnt/tiger/home/test_db/comb/test_pat/cs_test/");
  strcat(filename,argv[1]);
  strcat(filename,".pat");
  ...
  test_arrays = (test_ptns/(sizeof(long long int)*8)) + 1;

  if((test_ptn = malloc(test_arrays*sizeof(long long int *)))==NULL){
    printf("test ptn malloc Error.\n");
    exit(-1);
  }
  for(i=0;i<test_arrays;i++){
    if((test_ptn[i] = malloc(input_lists*sizeof(long long int))) == NULL){
      printf("test ptn malloc Error.\n");
      exit(-1);  
    }    
  }
  ...
  for(i=0;i<test_arrays;i++){
    memset(test_ptn[i],0b0,input_lists*sizeof(long long int)); //すべて０で初期化
  }
  ...
  for(i=0;i<test_ptns;i++){
	...
	if(input_rd == 1 || input_rd == 0){
	  test_ptn[(i/(sizeof(long long int)*8))][j] = test_ptn[(i/(sizeof(long long int)*8))][j] << 1;
	  if(input_rd == 1){
	    test_ptn[(i/(sizeof(long long int)*8))][j]++;
	  }
	}else{
	  printf("pat File read Error.\n");
	  exit(-1);
  ...
  //正しい出力を記録する配列を確保
  if((true_out = malloc(test_arrays*sizeof(long long int *)))==NULL){
    printf("true out malloc Error.\n");
    exit(-1);
  }
  for(i=0;i<test_arrays;i++){
    if((true_out[i] = malloc(signal_lines*sizeof(long long int))) == NULL){
      printf("true out malloc Error.\n");
      exit(-1);  
    }    
  }
  ...
  //並列計算用の配列を確保
  signal_bit = malloc(signal_lines*sizeof(long long int));
  ...
  for(ptn_i=0;ptn_i<test_arrays;ptn_i++){
    ...
    //出力をtrue_out に保存
    memcpy(true_out[ptn_i],signal_bit,sizeof(long long int)*signal_lines);
  ...
  //故障パターンファイル読み込み
  strcpy(filename,"/mnt/tiger/home/test_db/comb/benchmark/iscas89_cs/Faultset/");
  strcat(filename,argv[1]);
  ...
  for(fault_i=0;fault_i<fault_ptns;fault_i++){
    ...
    for(ptn_i=0;ptn_i<test_arrays;ptn_i++){
      //信号線出力を正常状態から読み出す
      memcpy(signal_bit,true_out[ptn_i],sizeof(long long int)*signal_lines);

      //故障信号線の定義
      //signal_line[(fault_ptn[fault_i][0]-1)][5] = fault_ptn[fault_i][1];
      if(fault_ptn[fault_i][1] == 0 || fault_ptn[fault_i][1] == 1){
	signal_bit[(fault_ptn[fault_i][0]-1)] ^= signal_bit[(fault_ptn[fault_i][0]-1)]; //すべてを０へ
	if(fault_ptn[fault_i][1] == 1){
	  signal_bit[(fault_ptn[fault_i][0]-1)] = ~signal_bit[(fault_ptn[fault_i][0]-1)]; //すべてを1へ
	}
      }else{
	printf("fault ptn read Error.\n");
	exit(-1);
      }
      i = 0;
	...
	i++;
      ...
      //出力をtrue_outと比較
      for(i=0;i<output_lists;i++){
	if(true_out[ptn_i][(output_list[i]-1)] != signal_bit[(signal_line[(output_list[i]-1)][4])-1]){
	  found_faults ++;
	  flg = 1;
	  break;
	}
...
void logic_cal_bit
(int (*signal_line)[6],long long int *signal_bit,int line_no,unsigned int *pointa_list){
  //信号線の出力をビット並列化で計算
  long long int res;
  int i;
  int pointa_head;
