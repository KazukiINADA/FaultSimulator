...
int main (int argc, char *argv[]){
  ...
  test_arrays = (test_ptns/(sizeof(int)*8)) + 1;

  if((test_ptn = malloc(test_arrays*sizeof(int *)))==NULL){
    printf("test ptn malloc Error.\n");
    exit(-1);
  }
  for(i=0;i<test_arrays;i++){
    if((test_ptn[i] = malloc(input_lists*sizeof(int))) == NULL){
      printf("test ptn malloc Error.\n");
      exit(-1);  
    }    
  }
  ...
  for(i=0;i<test_ptns;i++){
	...
	if(input_rd == 1 || input_rd == 0){
	  test_ptn[(i/(sizeof(int)*8))][j] = test_ptn[(i/(sizeof(int)*8))][j] << 1;
	  if(input_rd == 1){
	    test_ptn[(i/(sizeof(int)*8))][j]++;
	  }
	}else{
	  printf("pat File read Error.\n");
	  exit(-1);
  ...
  //正しい出力を記録する配列を確保
  if((true_out = malloc(test_arrays*sizeof(int *)))==NULL){
    printf("true out malloc Error.\n");
    exit(-1);
  }
  for(i=0;i<test_arrays;i++){
    if((true_out[i] = malloc(signal_lines*sizeof(int))) == NULL){
      printf("true out malloc Error.\n");
      exit(-1);  
    }    
  }
  ...
  //並列計算用の配列を確保
  signal_bit = malloc(signal_lines*sizeof(int));
...
void logic_cal_bit
(int (*signal_line)[6],int *signal_bit,int line_no,unsigned int *pointa_list){
  //信号線の出力をビット並列化で計算
  int res;
  int i;
  int pointa_head;
