...
void logic_cal_bit(int (*signal_line)[6],unsigned long long int *signal_bit,int line_no,unsigned int *pointa_list);
...
int main (int argc, char *argv[]){
  ...
  unsigned long long int **test_ptn;
  ...
  unsigned long long int *signal_bit;
  unsigned long long int mask_bit;
  
  unsigned int found_faults;
  char flg;
  int set_faults;
  unsigned long long int faults_sum;
  ...
  if((test_ptn = malloc(test_ptns*sizeof(unsigned long long int *)))==NULL){
    printf("test ptn malloc Error.\n");
    exit(-1);
  }
  for(i=0;i<test_ptns;i++){
    if((test_ptn[i] = malloc(input_lists*sizeof(unsigned long long int))) == NULL){
      printf("test ptn malloc Error.\n");
      exit(-1);  
    }    
  }
  ...
  for(i=0;i<test_ptns;i++){
	...
	if(input_rd == 1 || input_rd == 0){
	  test_ptn[i][j] = (unsigned long long int)0b0;
	  if(input_rd == 1){
  ...
  //並列計算用の配列を確保
  signal_bit = malloc(signal_lines*sizeof(unsigned long long int));
  ...
  for(ptn_i=0;ptn_i<test_ptns;ptn_i++){
	...
	if(now_p != NULL){
	  if(now_p->signal_no == i && set_faults < (sizeof(unsigned long long int)*8)){
	    //故障の設定
	    if(now_p->fault_ptn == 0){
	      mask_bit = ~((unsigned long long int)1<<set_faults);
	    ...
	    if(now_p->next != NULL && now_p->next->signal_no == i && set_faults < ((sizeof(unsigned long long int) *8)-1)){
	      //故障の設定
	      if(now_p->next->fault_ptn == 0){
		mask_bit = ~((unsigned long long int)1<<(set_faults+1));
		signal_bit[cal_no[i]-1] &= mask_bit;
	      }else{ //1縮退
		mask_bit = ((unsigned long long int)1<<(set_faults+1));
		signal_bit[cal_no[i]-1] |= mask_bit;
	...
	faults_sum |= (signal_bit[signal_line[(output_list[i]-1)][4]-1]);
      ...
      for(i=1;i<(sizeof(unsigned long long int)*8);i++){
	if(head_p == NULL){
	  break;
	}
	if(faults_sum & ((unsigned long long int)1<<i)){
	  //故障発見
	  if(head_p->prev == NULL){
	    start_p = head_p->next;
	    if(head_p->next == NULL){
